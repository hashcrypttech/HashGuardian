    ...
    <div class="wd-card">
        @if($entries->isEmpty())
            <div class="wd-empty">
                ...
                <div class="wd-empty-text">There are no outgoing requests to display.</div>
            </div>
        @else
            <table class="wd-table">
                ...
                <tbody>
                    @foreach($entries as $entry)
                        ...
                        <tr>
                            <td>{{ $c['method'] ?? 'N/A' }}</td>
                            <td><code class="wd-code-inline">{{ \Illuminate\Support\Str::limit($c['url'] ?? 'N/A', 80) }}</code></td>
                            <td>
                                @php $status = $c['status'] ?? null; @endphp
                                @if($status !== null)
