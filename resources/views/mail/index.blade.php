    ...
    <div class="wd-card">
        @if($entries->isEmpty())
            <div class="wd-empty">
                ...
                <div class="wd-empty-text">There are no mail events to display.</div>
            </div>
        @else
            <table class="wd-table">
                ...
                <tbody>
                    @foreach($entries as $entry)
                        ...
                        <tr>
                            <td>{{ $c['mailable'] ?? 'N/A' }}</td>
                            <td>{{ \Illuminate\Support\Str::limit($c['subject'] ?? 'N/A', 60) }}</td>
                            <td>{{ \Illuminate\Support\Str::limit(is_array($c['to'] ?? null) ? implode(', ', $c['to']) : ($c['to'] ?? 'N/A'), 50) }}</td>
